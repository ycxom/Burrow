...
/// ## 它挡的是哪一层
///
/// 落盘那一层**已经加密了** —— 整个数据库的内容列都用 app 密码派生的密钥
/// 加过（见 data/db_cipher.dart），拿到 `burrow.db` 的人本来就读不出任何
/// 一句话。那一层不归这道锁管。
///
/// 这道锁加的是**另一层**：手机已经解锁、app 已经打开、数据库密钥已经在
/// 内存里的那一刻 —— 手机借出去的那几分钟、放在桌上没锁屏的那一会儿。
///
/// ## 它**不是**第二把钥匙
///
/// 锁着的会话和别的会话用的是同一把数据库密钥。所以知道 app 密码的人
/// 仍然读得到它的内容（绕开界面直接读库就行）。这道锁是界面上的一道门，
/// 不是密码学上的隔离。
///
/// 为什么不给它单独一把钥匙：**找回方式决定了这件事做不到**。真按密码派生
/// 密钥的话，忘了就是永久打不开；而这里要求"忘了可以靠安全问题找回"，还要求
/// 那些答案是**模糊匹配**的 —— 模糊匹配的输入推不出稳定的密钥，差一个字就是
/// 完全不同的密钥。两个要求不可能同时满足。
///
/// 所以界面上一律叫「私密对话」「锁」，不叫加密：用户得知道它多挡了什么、
/// 又没多挡什么，才好决定往里面放什么。
library;
...
class ThreadLock {
  ...
  /// 题面和答案都得存，而答案还得是**原文**（模糊匹配要拿原文来比，
  /// 哈希之后就只能精确匹配了）。它跟着 lock_json 一起被数据库那层加密，
  /// 但对拿得到 app 密码的人是可读的。
