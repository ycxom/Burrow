...
///
///   - `history` 是模型看到的完整上下文，含 tool 消息；
///   - `_visible` 是人看到的气泡，滤掉了 tool，而且一个回合里模型分几次
///     写出的正文会被合成**一条**气泡。
///
/// 所以既不能直接用下标，也不能简单地"数偏移"。
library;
...
///
/// 助手消息不兜底是刻意的：一条助手气泡可能对应 history 里的好几条（工具
/// 循环里边说边调），按序号猜必然对不上，而对不上的后果是截断到错误的位置。
/// 宁可返回 -1 让调用方放弃。
