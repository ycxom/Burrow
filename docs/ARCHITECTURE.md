...
审批档位（§8）管的是「这次调用放不放行」，终端模式管的是**更前面一层**：
这一轮到底给不给模型工具。两者不重叠 —— 关掉终端模式时模型连
tool schema 都收不到，审批档位根本没有机会介入。

```dart
tools: terminalMode ? allToolSpecs : const <ToolSpec>[]
```

为什么不用「只读档 + 提示词里说别用工具」代替：

- **模型会为了用工具而用工具。** 只要 schema 在，「解释一下 Rust 的所有权」
  也可能先 `ls` 一遍 workspace。提示词压不住这个倾向，收走工具可以。
- 一整套 schema 每轮上千 token。日常聊天完全是白付的。
- 每轮开头的检查点要扫一遍 workspace。聊天模式下那一轮没有任何东西
  能改文件，扫描是纯浪费。

配套的两件事：

1. **系统提示分两份。** 聊天模式那份必须明说「你现在没有工具」——
   不说的话模型会按训练里的习惯承诺「我来帮你跑一下」然后什么都没发生，
   用户看到的是一个会撒谎的助手。
2. **兜底拒绝。** 本地小模型有时会硬编一个 tool call 出来。这时返回一条
   普通的拒绝消息（而不是抛异常）：抛出去整个回合就废了，返回一句话
   模型下一轮就会改用文字回答。

开关跟着**会话**走（`threads.terminal_mode` 列），不是全局的：
「帮我看看这段代码什么意思」和「把这个仓库编出来」是两种对话。
新会话的初值取上一次的选择。

### 7.2 运行中挂载基座

终端模式是模型碰得到沙箱的唯一入口，所以「第一次勾选」也就成了唯一
需要向用户解释「发行版基座」这个概念的时刻 —— 没装就当场推安装页。

装完必须**立刻生效**。要求重启 app 会把「勾一下就能用」变成
「勾一下、等下载、再手动杀进程重开」，那条路没人会走完。三处要一起换，
漏一个就是一类静默错误：

| 换什么 | 为什么 |
|---|---|
| `SandboxSession.rootfsPath` | `buildArgv` / `buildEnv` 每次调用都重读，换完下一条命令就落在新 rootfs |
| `PrefixGenerations.filesRoot` | 代目录跟着 rootfs 走。沿用旧列表会让时间线显示出另一个发行版的代，点回滚会 rename 到不存在的目录 |
| 交互 shell | argv 在 `spawn` 那一刻就定死了，只能杀掉重开 |

新建的会话另有一条路：`buildRuntime` 每次都重读一个
`ValueNotifier<InstalledDistro?>`，所以装完之后新开的对话也看得到。
