...
- **发送图片**：可从相册选图或直接拍照；对话模型看不了图时，
  可以先让视觉模型把图描述成文字再交给它。
- **模型分工**：对话、嵌入、图片转文字、摘要可以各用各的渠道 ——
  聊天走 A、嵌入走 B、图片转文字走 C，每一项都发往它自己那个渠道。
...
「设置 → 模型分工」里可以为**每一件事**分别指定渠道和模型：对话、嵌入（记忆检索）、
图片转文字、摘要各挑各的，互不影响。指派存的是**渠道 + 模型这一对**，所以嵌入指到 B
之后，它的地址、密钥和代理也跟着 B 走 —— 而不是拿 B 的模型名去当前渠道上请求。
没单独指定的项跟着当前对话渠道走。

> 嵌入和摘要走的是 OpenAI 兼容的 `/embeddings` 和 `/chat/completions`，**不跟着渠道协议变**。
> 把它们指到 Anthropic 或 Gemini 原生渠道上，分工表里那一行会直接标出来。
