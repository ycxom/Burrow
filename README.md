...
**模型列表跟着渠道存**（`burrow.llm.modelsByChannel`）。曾经是一份全局缓存，
那是个会让人花错钱的设计：在 A 渠道拉一次、切到 B，选择器里列的还是 A 的
模型 —— 挑一个发出去，要么 404，要么更糟：B 那边刚好有个同名模型，
于是照常计费。渠道删掉时它那份列表一起清掉。
...
| **渠道 + 每渠道代理** | 模拟器上：旧配置自动迁成「OpenAI」渠道（地址、模型、密钥都在）。同一个渠道**直连时「连接正常」，把代理指向没人监听的 127.0.0.1:9 后变成「Connection refused」** —— 这是代理真的作用到了请求上的确证，而不是只存了个字段。清掉代理后恢复正常 |
...
| **模型的来源标注** | 模拟器上建了第二个渠道（LAN-228，指向一个不通的网关），两个渠道**都配着叫 `GLM-5` 的模型** —— 没有渠道名的话这两种状态在界面上完全一样。实测：芯片和顶栏一起变成 `LAN-228 · GLM-5`；选择器里两个来源并排，翻到非当前来源时出现「会把当前渠道一起切到「OpenAI」，之后的对话都发往 192.168.36.133:3000」，且**那边的 `GLM-5` 没有打勾**；LAN-228 的拉取超时只写在它自己名下，OpenAI 的 57 个缓存模型不受影响（**这一条同时证明了模型缓存是按渠道分开存的**）。点它 → 渠道跟着切走 → 发一轮，气泡署名 `OpenAI · GLM-5`；**再切回 LAN-228，那条历史气泡仍然是 `OpenAI · GLM-5`**。删掉测试渠道后 prefs 里它那份模型列表也一起没了 |
| **图片 + 前置多模态端到端** | 模拟器上：推一张带文字的测试截图（内含 `PANGOLIN-7731`）到相册 → 相册选图 → 缩略图托盘 → 发送。**没配视觉模型时**报「没有可用的视觉模型。到「渠道管理」里…」并且附件图标是警告色；给渠道填上视觉模型后图标恢复正常、副标题变成「当前模型不认图，会先用视觉模型描述一遍」。真实网关上试了三个模型，错误逐条如实透出（`GLM-5` → `HTTP 400 Format Error`；`Qwen3.8-Max` 第一次 → `HTTP 500 upstream error`），换 `Qwen3.8-Max` 重试后**对话模型 `mimo-v2.5`（不认图）答出了 `PANGOLIN-7731`** —— 它只可能来自视觉模型的描述，这是前置多模态生效的确证。气泡署名 `Test · mimo-v2.5` |
...
改漏一个就是一串对不上号的报错。现在能存多个随时切。

- **代理是每个渠道各自的**，不是全局的。一个渠道走内网直连、另一个走梯子
  是很常见的组合，全局代理会把前者也拖进去。代理设在 `HttpClient` 上而不是
  单个请求的参数，所以**换配置时代理变了必须重建客户端** —— 不重建的表现是
  「改了代理没反应」。`cancel()` 重建时也要带上代理，否则会出现
  "只在取消过一次之后才消失"的代理。
- **OAuth 登录并进了渠道管理**，不再是单独一页「模型账号」。一个 OAuth 账号
  本身没有用处，它只有作为某个渠道的认证方式才有意义；分成两页的话，
  用户登录完会停在一个"然后呢"的界面上。
- **同一个服务商可以登录多个账号**，每个账号绑到不同渠道上，切渠道就等于
  切账号。账号 id 优先用邮箱 —— 用户唯一认得出来的东西。
- **渠道只存"用哪个账号"，不存 token**。access_token 会过期，抄进配置就是
  抄了一份马上失效的副本；真正的 token 由 `bearerProvider` 每次请求前现取。
- 旧的单份配置会**自动迁成第一个渠道**（OAuth 的单账号存储同理）。
  不迁的话升级后看到的是一个空列表，那看起来就是"我的配置丢了"。

### 模型的来源标注

同一个模型名同时挂在一个免费网关和一个计费官方接口上，是最常见的配置，
不是什么边角情况。所以只显示 `GLM-5` 是**认不出花谁的钱的** ——
用户以为在走自己的网关，实际每一轮都在扣官方额度，而这种事往往要等到
账单出来才发现。

- 芯片、顶栏副标题、助手气泡的署名一律是 `渠道名 · 模型名`。
  **只有一个渠道时这些前缀全部收起来** —— 没有第二个来源要区分，
  那就只是噪音。
- 选择器里**先选来源再选模型**。来源那一排放在列表上面而不是折进下拉：
  「在哪个来源上挑」是选模型时一直要看得见的前提，不是二级选项。
- 每个来源下面写的是**地址**（`host:port`）而不只是渠道名。名字是用户
  自己起的，可能叫「测试」「新渠道2」；花的是谁的额度由地址决定。
- 翻到别的来源时，**同名模型不打勾** —— 打上勾会让人以为"已经是它了"
  从而放心点下去。跨来源挑模型会先明说「这会把当前渠道一起切过去，
  之后的对话都发往 x.x.x.x」，说在点下去之前，不是切完在状态条上闪一句。
- 助手气泡的署名**存在消息里**（`messages.source`，v4 迁移），不是显示时
  现取当前配置。换个渠道就把满屏历史全部改署成新渠道的话，恰好会在用户
  回头查"刚才那次是谁花的额度"时给出错误答案。老消息没有这个记录就不署名，
  而不是补一个多半是错的章。

`ChannelStore.newId()` 原来只用微秒时间戳，单测里连生成 200 个只得到
3 个不同值 —— `microsecondsSinceEpoch` 的实际精度看平台，Windows 上是毫秒级。
撞了的后果是 `upsert` 把前一个渠道整个覆盖掉，不报错，只是少了一个渠道。
现在加了单调计数器。

### 图片：直发与前置多模态

两条路，输入框左边那个图标的颜色就说明现在走哪条：

- **直发**：图片以 base64 内联进请求体。三家协议的块形状都不一样
  （OpenAI 的 `image_url` 嵌套对象 / Anthropic 的 `source.media_type+data` /
  Responses API 的 `input_image` 平铺字符串），照搬一家的形状给另一家会被直接拒。
- **前置多模态**（思路来自 VPetLLM 的 `PreprocessingMultimodal`）：先让一个
  视觉模型把图**描述成文字**，描述作为一条 system 消息插在用户那句话前面。
  主力模型因此不必是多模态的；而且描述进上下文之后一直是文本 ——
  原生多模态每一轮都要把图重新上传一遍，描述只占几百 token，
  还能被滚动摘要正常压缩。代价是**有损**：描述模型没提到的细节就永远没了。

几个刻意的决定：

- **类型看文件头，不看扩展名**。截图工具存出来的 `.jpg` 其实是 PNG 非常常见；
  跟着扩展名填 `media_type`，Anthropic 直接 400，而有的网关照单全收，
  模型看到的是一张花屏然后一本正经地描述它。
- **`sendImagesInline` 一个开关管两件事**：模型认不认图（渠道上手动勾），
  以及用户有没有强制走前置（省钱）。不认图时连读盘都跳过。
- **没有图的消息仍然发字符串 content**，不是单元素数组 —— 不少本地推理服务
  （llama.cpp、部分 vLLM）只认字符串，一律改成数组会把它们全打挂。
- **多个视觉候选、随机挑、挂了换下一个**，全挂了把每个候选各自的原因一起报出来。
  视觉是可选增强，不该因为某个网关抽风就让整条消息发不出去。
- **送不到就明确报错，不静默丢图**。丢掉的话模型会对着一条提到图却没有图的
  消息硬答，而用户完全看不出来。
- 选图时就缩到 1600px / JPEG 85，在平台层做。手机原图 4MB，base64 之后 5.3MB，
  而且历史里的图每一轮都要重传。
